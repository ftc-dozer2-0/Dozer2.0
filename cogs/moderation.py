...
class Moderation(Cog):
    ...
    async def mod_log(self, actor: discord.Member, action: str, target: Union[discord.User, discord.Member, None],
                      ...
                      updated_by: discord.Member = None):
    ...
    async def perm_override(self, member: discord.Member, **overwrites):
        ...
        for channel in channels:
            ...
            if channel.permissions_for(me).manage_roles:
                ...
                try:
                    await channel.set_permissions(target = member,
    ...
    async def punishment_timer(self, seconds: int, target: discord.Member, punishment, reason: str,
        ...
        try:
            if len(user) != 0:
                ...
                logger.warning(
                    f"User {target} was not found in the {punishment.__name__} database, skipping un-punishment")
        ...
        for subscription in subscriptions:
            ...
            if sub_guild:
                ...
                try:
                    ...
                    if modlog_channel:
                        await self.mod_log(actor = ctx.message.author, action = "crossbanned", target = user,
                                           reason = reason,
                                           dm = False,
    ...
    async def warn(self, ctx: DozerContext, member: discord.Member, *, reason: str):
        ...
        await self.mod_log(actor = ctx.author, action = "warned", target = member, orig_channel = orig_channel,
                           reason = reason)
    ...
    async def timeout(self, ctx: DozerContext, duration: float):
        ...
        e = discord.Embed(title = f'Timeout - {duration}s',
                          description = 'This channel has been timed out. Do not timeout the channel again until current timeout has ended.',
    ...
    @commands.hybrid_command()
    @has_permissions(administrator = True)
    @app_commands.describe(channel_mention = "The channel to set as the new members channel",
    ...
    @commands.hybrid_command()
    @has_permissions(administrator = True)
    @app_commands.describe(role = "the role used for the new members purge",
                           days = "the number of days after which to kick members")
    ...
    @has_permissions(administrator = True)
    @bot_has_permissions(ban_members = True)
    @app_commands.describe(guild_id = "The server you'd like to subscribe to bans from")
    ...
    @crossbans.command()
    @has_permissions(administrator = True)
    @app_commands.describe(guild_id = "The server you'd like to unsubscribe from for ban data")
